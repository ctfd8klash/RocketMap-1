    ...
    if wild_pokemon and not args.no_pokemon:
        ...
        for p in wild_pokemon:
            ...
            if args.ignoremaybe_file and any(poke[0] == pokemon_id for poke in args.maybelist):
                if randint(1, 100) <= args.maybelist[[x[0] for x in args.maybelist].index(pokemon_id)][1]:
                    log.debug('Ignoring Pokemon id: %i with odds of %i', pokemon_id, args.maybelist[[x[0] for x in args.maybelist].index(pokemon_id)][1])
                    filtered += 1
                    continue
            ...
            if args.encounter and (pokemon_id in args.enc_whitelist):
                if args.pgscout_url and level < 30:
                    ...
                    pokemon_info = encounter_pokemon(args, p, account, api,
                                                     account_sets, status,
                                                     key_scheduler)
