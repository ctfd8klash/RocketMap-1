...
//jmk        searchMarker = createSearchMarker()
    }
        ...
        if (isUpcomingRaid) {
            imageLbl = `
                <div class='raid'>
                  ...
                  Raid in <span class='raid countdown label-countdown' disappears-at='${raid.start}'> (${moment(raid.start).format('h:mm a')})</span> (${moment(raid.start).format('h:mm a')})
                </div>`
        }
    } else {
        ...
        str = `
            <div>
              ...
              <div class='pokestop-expire'>
                  <span class='label-countdown' disappears-at='${expireTime}'>00m00s</span> left (${moment(expireTime).format('h:mm a')})
              </div>
              <div>
...
function clearStaleMarkers() {
    ...
    $.each(mapData.pokemons, function (key, pokemon) {
        ...
        const isPokeExcluded = (excludedPokemon.indexOf(pokemonId) !== -1) || (pokemonId === 16)  // force hide pidgeys JMK
...
function processPokemon(item) {
    const isPokeExcluded = (excludedPokemon.indexOf(item['pokemon_id']) !== -1) || (item['pokemon_id'] === 16)
...
function changeLocation(lat, lng) {
    ...
    changeSearchLocation(lat, lng).done(function () {
        ...
        locationMarker.setPosition(loc) // JMK

        if (searchMarker) {
//JMK            searchMarker.setPosition(loc)
        }
    })
}
